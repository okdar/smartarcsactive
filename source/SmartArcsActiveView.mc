...
class SmartArcsActiveView extends WatchUi.WatchFace {
    ...
    function onUpdate(dc) {
        ...
        //devices without partial-update support animate the second hand dot through onUpdate while awake
        if (isAwake && showSecondHand > 0 && !partialUpdatesAllowed) {
            drawSecondHandDot(targetDc, clockTime.sec);
        }

        //output the offscreen buffers to the main display if required.
        drawBackground(dc);

        //partial updates refresh HR and the second hand dot every second:
        // - "always" (2): dot in both awake and sleep modes
    ...
    function computeConstants() {
        ...
        rcN30 = recalculateCoordinate(-30);
        rcN40 = recalculateCoordinate(-40);
        rc70 = recalculateCoordinate(70);
        ...
        activity1Y = screenRadius + rc10;
        activity2Y = activity1Y + Graphics.getFontAscent(font);
        activityArcY = activity1Y + 1 + halfFontHeight;

        bbStressActivity1Y = screenRadius - (activity1Y - screenRadius + (halfFontHeight * 2)) - rc9;
    ...
    //draws the second hand as a small dot between the minute hand tip and the minute ticks
    function drawSecondHandDot(dc, seconds) {
        var dot = computeSecondHandDot(seconds);
    ...
    function onPartialUpdate(dc) {
        ...
        var refreshHR = false;
        if (hrColor != offSettingFlag) {
            if (hrRefreshInterval == 1 || clockSeconds % hrRefreshInterval == 0) {
                refreshHR = true;
            }
        }

        //erase the second hand dot drawn in the previous partial update (its area is stored in curClip)
        if (!fullScreenRefresh && curClip != null) {
            dc.setClip(curClip[0][0], curClip[0][1],
                curClip[1][0] - curClip[0][0] + 1, curClip[1][1] - curClip[0][1] + 1);
            drawBackground(dc);
        }
        curClip = null;

        //draw HR in its own small clip region
        if (hrColor != offSettingFlag) {
            ...
            //debug rectangle around the HR clip
            // dc.setPenWidth(1);
            // dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
            // dc.drawRectangle(screenRadius - halfHRTextWidth, rc30, hrTextDimension[0], hrTextDimension[1]);
        }

        //draw the second hand dot in its own small clip region
        if (showSecHandNow) {
            var dot = computeSecondHandDot(clockSeconds);
            var r = secondHandDotRadius + 1;
            var boxX = dot[0] - r;
            var boxY = dot[1] - r;
            var boxSize = 2 * r + 1;
            curClip = [[boxX, boxY], [boxX + boxSize - 1, boxY + boxSize - 1]];

            dc.setClip(boxX, boxY, boxSize, boxSize);
            drawBackground(dc);
            dc.setColor(getSecondHandColor(), Graphics.COLOR_TRANSPARENT);
            dc.fillCircle(dot[0], dot[1], secondHandDotRadius);

            //debug rectangle around the second hand dot clip
            // dc.setPenWidth(1);
            // dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
    ...
    function drawSteps(dc, steps, stepsGoal, distance, distanceUnits) {
        ...
        drawActivity(dc, STEPS, steps, distance, stepsGoal, rc70, activity1Y, activity2Y, activityArcY);
    }
    ...
    function drawFloors(dc, floorsClimbed, floorsDescended, floorsGoal) {
        ...
        drawActivity(dc, STAIRS, floorsClimbed, floorsDescended.toString(), floorsGoal, rcN30, activity1Y, activity2Y, activityArcY);
    }

    function drawBodyBattery(dc) {
        drawActivity(dc, BODY_BATTERY, getLastHistoryValue(Toybox.SensorHistory.getBodyBatteryHistory({:period=>1})), -1, 100, rc40, bbStressActivity1Y, -1, bbStressArcY);
    }

    function drawStress(dc) {
        drawActivity(dc, STRESS, getLastHistoryValue(Toybox.SensorHistory.getStressHistory({:period=>1})), -1, 100, rcN40, bbStressActivity1Y, -1, bbStressArcY);
    }
    ...
    function drawActivity(dc, activityType, activity1, activity2, goal, xShift, activity1Y, activity2Y, activityArcY) {
        var progressXShift = xShift - rc5;

        dc.setColor(activityColor, Graphics.COLOR_TRANSPARENT);
        if (activity1 > 0 || (activityType <= STAIRS && activity2.toFloat() > 0) || showZero) {
            dc.setPenWidth(1);
        ...
        if (activityType <= STAIRS && (activity2.toFloat() > 0 || showZero)) {
            dc.setColor(activityColor, Graphics.COLOR_TRANSPARENT);
            dc.drawText(screenRadius - xShift + rc20, activity2Y, font, activity2, Graphics.TEXT_JUSTIFY_LEFT);
        }
    }
    ...
    function drawHR(dc, refreshHR) {
        ...
        //clip and debug rectangle are managed by the caller (onPartialUpdate)
        dc.setColor(hrColor, Graphics.COLOR_TRANSPARENT);
        dc.drawText(screenRadius, rc30, font, hrText, Graphics.TEXT_JUSTIFY_CENTER);
    }
	...
	function computeSunConstants() {
    	...
    	if (posInfo != null && posInfo.position != null) {
            ...
            var dawn = sc.calculate(time_now, loc, SunCalc.DAWN);
            var sunrise = sc.calculate(time_now, loc, SunCalc.SUNRISE);
            var sunset = sc.calculate(time_now, loc, SunCalc.SUNSET);
            var dusk = sc.calculate(time_now, loc, SunCalc.DUSK);

            if (dawn == null || sunrise == null || sunset == null || dusk == null) {
                //polar day or polar night — no valid sun times
                sunriseStartAngle = 0;
                sunriseEndAngle = 0;
                sunsetStartAngle = 0;
                sunsetEndAngle = 0;
